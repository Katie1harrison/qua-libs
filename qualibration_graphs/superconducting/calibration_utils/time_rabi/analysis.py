...
@dataclass
class FitParameters:
    """Stores the time Rabi fit results for a single qubit."""

    pi_duration_ns: float
    """Pi-pulse duration in nanoseconds (rounded to nearest 4 ns)."""
    num_periods: float
...
def log_fitted_results(fit_results: Dict, log_callable=None):
    ...
    for q, result in fit_results.items():
        ...
        s_dur = f"\tPi-pulse duration: {result['pi_duration_ns']:.0f} ns | "
...
def _fit_oscillation_time(da: xr.DataArray, dim: str) -> xr.DataArray:
    ...
    The standard qualibration_libs fit_oscillation applies a hard `xf > 0.1`
    frequency mask that discards any Rabi signal with a period longer than
    ~40 ns (10 clock cycles).  For typical time-Rabi sweeps the oscillation
    period is hundreds to thousands of nanoseconds, so that mask eliminates
    the real signal and the fitter locks onto noise harmonics.

    This function removes the mask, estimates the dominant frequency from the
    full positive-frequency FFT spectrum, and fits within bounds centred on
    that estimate.
    """
    ...
    def _fit_single(x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        T = float(x[1] - x[0])  # step size in clock cycles

        # FFT — no frequency lower-bound cutoff
        yf = np.fft.rfft(y_sub)
        xf = np.fft.rfftfreq(N, T)  # frequencies in units of 1/cc

        # Skip DC bin (index 0) only
        ...
        # Bounds: allow ±factor around the FFT guess, capped at Nyquist
        nyquist = float(xf[-1])
...
def _compute_chi2(fit: xr.Dataset) -> xr.DataArray:
    ...
    chi2 ≤ 2 → oscillation detected; chi2 > 2 → residuals dominate (no Rabi signal).
    """
...
def fit_raw_data(
    ...
    """
    Fit Rabi oscillations in I vs duration_cc for each qubit.
    ...
    for q in ds_fit.qubit.values:
        ...
        # π-pulse duration: half period in clock cycles, converted to ns
        if np.isfinite(f) and abs(f) > 0:
            pi_cc = 1.0 / (2.0 * abs(f))
            # Round to nearest clock cycle (4 ns)
