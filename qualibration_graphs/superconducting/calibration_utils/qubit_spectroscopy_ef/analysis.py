...
@dataclass
class FitParameters:
    """Stores the relevant qubit EF spectroscopy experiment fit parameters for a single qubit"""
...
def log_fitted_results(fit_results: Dict, log_callable=None):
    ...
    for q in fit_results.keys():
        ...
        s_freq = f"\tEF frequency: {1e-9 * fit_results[q]['frequency']:.3f} GHz | "
        ...
        s_x180 = f"To get the desired EF_x180 gate, the EF_x180 amplitude is updated to: {1e3 * fit_results[q]['x180_amp']:.1f} mV\n "
...
def process_raw_dataset(ds: xr.Dataset, node: QualibrationNode):
    ...
    # The QUA program sweeps the EF IF as: df + anharmonicity + IF_ge
    # so the actual RF = f_ge + df + anharmonicity.
    # The bottom axis therefore shows: detuning + RF_ge + anharmonicity.
    full_freq = np.array(
        [ds.detuning + q.xy.RF_frequency + q.anharmonicity for q in node.namespace["qubits"]]
    )
    ds = ds.assign_coords(full_freq=(["qubit", "detuning"], full_freq))
    ds.full_freq.attrs = {"long_name": "EF RF frequency", "units": "Hz"}
    return ds
...
def _extract_relevant_fit_parameters(fit: xr.Dataset, node: QualibrationNode):
    ...
    # Absolute EF frequency = f_ge + fit.position + anharmonicity
    full_freq_offset = np.array([q.xy.RF_frequency + q.anharmonicity for q in node.namespace["qubits"]])
    res_freq = fit.position + full_freq_offset
    rel_freq = fit.position
    fit = fit.assign({"res_freq": ("qubit", res_freq.data)})
    fit = fit.assign({"relative_freq": ("qubit", rel_freq.data)})
    fit.res_freq.attrs = {"long_name": "EF frequency", "units": "Hz"}
