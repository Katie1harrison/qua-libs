...
@dataclass
class FitParameters:
    """Stores the relevant EF Ramsey experiment fit parameters for a single qubit."""

    freq_offset: float
    """Frequency correction to apply to q.anharmonicity (Hz)."""
    decay: float
    """T2* on the EF transition (seconds)."""
    decay_error: float
    success: bool
...
def log_fitted_results(fit_results: Dict, log_callable=None):
    """Log the fitted EF Ramsey results for all qubits."""
...
def fit_raw_data(ds: xr.Dataset, node: QualibrationNode) -> Tuple[xr.Dataset, dict[str, FitParameters]]:
    """
    Fit the EF frequency detuning and T2* from the EF Ramsey oscillations.

    Identical fitting procedure to the GE Ramsey.  The freq_offset returned
    is the correction to apply to ``q.anharmonicity``.
    """
...
def _compute_chi2(ds_fit: xr.Dataset, node: QualibrationNode) -> xr.DataArray:
    """
    Compute a pooled residual chi-squared for the EF Ramsey fit across both detuning traces.

        chi2 = (SS_res_+ + SS_res_-) / ((N - P) * (a_+² + a_-²))

    chi2 ≤ 2  →  oscillation detected; chi2 > 2  →  fit failed.
    """
...
def _extract_relevant_fit_parameters(fit: xr.Dataset, node: QualibrationNode):
    """Extract fit results — identical logic to the GE Ramsey."""
    ...
    tau = 1 / decay
    tau.attrs = {"long_name": "EF T2*", "units": "uSec"}

    tau_error = tau * (np.sqrt(decay_res) / decay)
    tau_error.attrs = {"long_name": "EF T2* error", "units": "uSec"}

    detuning = int(node.parameters.frequency_detuning_in_mhz * 1e6)

    freq_offset, decay, decay_error = _calculate_fit_results(frequency, tau, tau_error, fit, detuning)
...
def _calculate_fit_results(frequency, tau, tau_error, fit, detuning):
    """Calculate freq_offset, decay, and decay_error from fitted parameters."""
